...
\begin{frame}[fragile]{Turing e Church}
...
\begin{frame}[fragile]{IA e a grande questão}

    \begin{center}
        {\it \Large A linguagem natural poderia ser, de fato, a ``linguagem universal''?}

        \vspace{0.2in}
        {\it \Large Um LLM seria um método de decisão para a solução de todos os problemas descritos em linguagem natural?}
    \end{center}

\end{frame}

\begin{frame}[fragile]{Problema simples + solução (Gemini)}

    \begin{figure}
        \centering
        \includegraphics[scale=0.5]{gemini.png}
    \end{figure}
...
\begin{frame}[fragile]{Problema simples + solução (Gemini)}
    ...
    \begin{figure}
        \centering
        \includegraphics[scale=0.4]{gemini2.png}
    \end{figure}
