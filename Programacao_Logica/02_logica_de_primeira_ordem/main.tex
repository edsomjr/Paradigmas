...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[anchor=west] (c) at (1.0, 4.0) { $\star$ \bbtext{Para diferenciar regras de fatos neste caso, observe que, em uma regra, ao menos} };

	\node[anchor=west] (c1) at (0.5, 3.5) { \bbtext{um dos argumentos deve ser uma variável:} };
...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[anchor=west] (b) at (1.0, 5.0) { $\star$ \bbtext{Generalizações são regras que casam com qualquer valor} };
