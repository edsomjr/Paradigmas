...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[anchor=west] (e) at (1.0, 0.5) { $\star$ \bbtext{O tratamento das regras recursiva se dá naturalmente pelo \bbenglish{backtracking}} };
...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[anchor=west] (b) at (1.0, 5.0) { $\star$ \bbtext{A unificação entre o objetivo e o padrão de uma regra recursiva define as relações} };
...
\node[draw,opacity=0] at (14, 8) {x};
	...
	\node[anchor=west] (d) at (1.0, 1.5) { $\star$ \bbtext{A declaração de uma estrutura é semelhante à declaração de um fato ou regra} };
